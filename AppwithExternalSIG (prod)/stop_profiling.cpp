...
int main() {
    try {
        ...
        // Send the USR2 signal to the process using -s option
        std::string command = "kill -s USR2 " + pid;
        if (system(command.c_str()) == -1) {
            std::cerr << "Failed to send USR2 signal." << std::endl;
            return 1;
        }

        std::cout << "Profiling has completed and output.png file generated." << std::endl;
